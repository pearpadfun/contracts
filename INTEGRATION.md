...
|---|---|---|
| PearpadFactory | `0xF2fe7D1eC701f69aEc294CA625d8520D4C2340c4` | Launches tokens, bonding-curve trading pre-migration |
| PearpadRouter | `0xf403be0A82d6400E5cb018A89FDeC6201AF02d98` | Swaps post-migration (wraps Uniswap V3 + 1% fee) |
| PearpadLocker | `0x9158045B28b72700D2E9652F0A7952d88C7294cD` | Holds locked LP; not needed for swapping |
...
Every token launches on a bonding curve inside the **Factory**. When the curve's real reserve hits 10,000 USDT0, it auto-migrates to a Uniswap V3 pool (1% tier) and curve trading stops. After that, all swaps go through the **Router**.

Phase check: `factory.curves(token)` returns `(ethReserve, tokenReserve, creator, migrated)`.
...
- Curve fee: 1.3% total (1% split 60/40 creator/treasury + 0.3% protocol).
- `buy` quote for UI: `getTokensOut(token, msg.value * 10000 / 10130)` (fee comes off msg.value first).
- `sell` payout for UI: `gross = getEthOut(token, tokensIn)`, user receives `gross - gross*130/10000`.
- Buys that would push the reserve past 10,000 USDT0 are capped and the excess refunded; a buy that lands exactly on target triggers migration in the same tx (more gas — don't set tight gas limits).
- Events for indexing: `Launched`, `Bought(token, buyer, ethIn, tokensOut, ethReserve, tokenReserve)`, `Sold(...)`, `Migrated`.
- Launching: `launch(name, symbol, metadata, maxFee)` payable — launch fee is currently **0.69 USDT0** (`launchFee()`, may change; pass it as `maxFee` guard). Any value beyond the fee executes the creator's first buy atomically.
...
const FACTORY = '0xF2fe7D1eC701f69aEc294CA625d8520D4C2340c4'
const ROUTER  = '0xf403be0A82d6400E5cb018A89FDeC6201AF02d98'
...
// ---- launch a token (0.69 USDT0 fee; extra value = creator first buy) ----
